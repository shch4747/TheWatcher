...
- Per batch (ADR-0012, superseding the earlier per-message *Choice* design): (1) fetch the channel's active **and** stale threads with their titles, summaries and last few Timeline lines; (2) route replies deterministically — a `replied_to_id` pointing at a message already on a thread page goes straight there, one pointing at a message in the same batch follows that message; (3) split what's left into chunks that fit `INGEST_MAX_PROMPT_TOKENS` and ask the Worker Model once per chunk for structured JSON: a thread per message (an existing slug, a `new-N` it declares with a title and 1–2 sentence description, or `chatter`), with every message shown as `id | sent time | sender name (jid) | reply target | text`; threads minted by one chunk are context for the next, so a topic spanning a boundary stays one thread; (4) the answer is validated against the ids and slugs actually in the prompt and re-asked with the problem stated if it doesn't hold; leftovers are dropped as chatter with a logs-channel notice rather than blocking the batch; (5) for each touched thread, one more structured call returns its refreshed title, rewritten Summary and typed Items, which are rendered into the item grammar in code (every line carrying `[src:: id]`, ungrounded items dropped); (6) new threads get id `<yyyymmdd>-<slug>`; (7) pages written through the wiki layer; channel index regenerated; cursor advanced; Update Notices posted to the Inbox of the initiative's agent (Project Agent for project/event channels; none for other kinds in v1); (8) chatter is dropped. The prompts for both calls are code constants, not skills.
- One thread per message. (The earlier multi-label idea — a message on several threads when the top-2 probabilities both cleared the threshold — went with the Choice path; the structured contract asks for exactly one.)
...
### Models
- Decision Model: Jev via a small in-house client (Choice/Noul/Score), batched per request, with a confidence threshold of 0.6 and Worker fallback. Used by the Chat Agent only since ADR-0012; ingestion is Worker-only. Worker Model: a cheap model, candidates to benchmark in Phase 6 (user's shortlist: GPT-6 Luna, GLM 5.3 Flash). Mentor Model: `claude-sonnet-5`. All behind one `models` interface so tiers are swappable per config; Pydantic AI agents take the model per constructor.
- Every model call is logged with tokens and cost to the run ledger. Since ADR-0013 each row also carries which channel and which ingestion phase (classification vs summarisation) it belonged to, its latency, its outcome, and whether the cost was reported by the provider, computed from tokens, or estimated — plus per-channel run timing in `obs_ingest_runs`/`obs_ingest_channel_runs`, an audit row per wiki write in `obs_vault_ops`, a cost report posted to the logs channel after each run that did work, a provisioned Grafana dashboard over the same SQLite file, and OpenTelemetry traces exported to a self-hosted Phoenix.
