...
def _feedback_context_real(vault: VaultClient, lane: str) -> str:
    reviewed = interface.get_reviewed_ideas_sync(vault, lane=lane)
    liked = [r for r in reviewed if r["verdict"] == "up"][:3]
    disliked = [r for r in reviewed if r["verdict"] == "down"][:3]
    if not liked and not disliked:
        return ""
    lines = ["\n\nPast human feedback on ideas from this lane:"]
    for item in liked:
        lines.append(f"- LIKED: {item['title']} — {item.get('reason') or 'no reason given'}")
    for item in disliked:
        lines.append(f"- REJECTED: {item['title']} — {item.get('reason') or 'no reason given'}")
    lines.append(
        "Use this to calibrate what kind of ideas land well with this audience. "
        "Don't propose ideas that closely repeat a REJECTED pattern."
    )
...
def grounded_on_closure(vault: VaultClient, project_path: str) -> list[Idea]:
    causes = interface.get_closed_project_causes_sync(vault)
    cause = causes.get(project_path)
    if not cause:
        return []
    matches = [p for p, c in causes.items() if c == cause and p != project_path]
    if not matches:
        return []
    prompt = (
        f"ARIES projects {matches + [project_path]} all failed for the same reason: '{cause}'. "
    ...
    ) + _feedback_context_real(vault, "remedial")
    return generate_ideas(prompt, Origin.REMEDIAL)


def grounded_weekly_sweep(vault: VaultClient) -> list[Idea]:
    usage = interface.get_capability_usage_sync(vault)
